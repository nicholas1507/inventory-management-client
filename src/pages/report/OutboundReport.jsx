...
import {getMovements} from '../../api/api';
...
const OutboundReport = () => {
    ...
    const load = async(page = 1, keyword = "", start = "", end = "", action ="") => {
        setLoading(true);
        setError(null);
        try{
            const data = await getMovements({page, limit: pagination.limit, search: keyword, type: "OUT", startDate: start, endDate: end, maker: action});
    ...
    useEffect(() => {
        load(currentPage,search,appliedFilter.startDate,appliedFilter.endDate,outbound);
    },[currentPage,search,appliedFilter,outbound]);
        ...
        <div>
            ...
            <div className="mb-3">
                ...
                >
                    ...
                    <option value={"User"}>Outbound Request</option>
                    <option value={"Admin"}>Outbound Admin</option>
                </select>
            </div>
            <div className="mb-3">
                <label className="form-label fw-semibold small text-uppercase text-muted">
